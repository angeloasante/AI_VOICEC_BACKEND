 ...
 */
function isVisaQuery(message: string): { isVisa: boolean; passport?: string; destination?: string; residence?: string } {
  ...
  for (const pattern of citizenshipPatterns) {
    ...
    if (match) {
      ...
      passport = parseCountryCode(nationality) || parseCountryCode(nationality.replace(/n$|an$|ian$|ish$|ese$|i$/i, ''));
      if (passport) break;
    }
  }
  ...
  for (const pattern of residencePatterns) {
    const match = lowerMessage.match(pattern);
    if (match) {
      const res = parseCountryCode(match[1]);
  ...
  if (!passport && !destination) {
    ...
    if (match) {
      const from = parseCountryCode(match[1]);
      const to = parseCountryCode(match[2]);
      if (from && to) {
        passport = from;
        destination = to;
      }
...
export async function generateResponse(
  ...
  if (forceApiCall) {
    ...
    markVisaApiCalled(streamSid);
    
    const visaResult = await checkVisaRequirements(fromCountry, toCountry);
    ...
    if (visaResult.success && visaResult.data) {
      ...
      addMessage(streamSid, 'assistant', fallbackResponse);
